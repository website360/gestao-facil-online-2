-- Atualizar política RLS para permitir que usuários de conferência também vejam os dados dos clientes
...
USING (
  (EXISTS ( 
    ...
    AND profiles.role = ANY (ARRAY['admin'::user_role, 'gerente'::user_role, 'separacao'::user_role, 'conferencia'::user_role])
  )) 
  OR 
  ((EXISTS ( 
    SELECT 1 FROM public.profiles 
